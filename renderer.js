...
function log(message) {
  ...
  while (logPanel.childElementCount > 120) {
...
function applyBrowserStatus(browser, options = {}) {
  ...
  if (!browser?.chromeAvailable) {
    ...
    browserSurfaceMessage.textContent =
      "Install Google Chrome in /Applications to use the real-browser monitoring mode.";
...
async function boot() {
  ...
  const browser = await window.monitorApi.getBrowserStatus();
  applyBrowserStatus(browser);
...
const settingsFields = document.querySelectorAll(".control-panel input, .control-panel select");
